...
class HomeController extends Controller
{
    public function homePage()
    {
        ...
        // first_sections
        $first_sections = Post::with(['category:id,category_name_bn,category_name_en', 'user:id,name'])->where('status', 1)->where('first_section', 1)->latest('id')->select('id', 'category_id', 'title_slug_en', 'title_slug_bn', 'user_id', 'title_en', 'title_slug_en', 'title_slug_bn', 'title_bn', 'thumbnail', 'created_at')->limit(2)->get();

        // firstsection big thumbnail
        $firstSectionBigThumbnail = Post::with(['category:id,category_name_bn,category_name_en', 'user:id,name'])->where('status', 1)->where('first_section_thumbnail', 1)->latest('id')->select('id', 'category_id', 'user_id', 'title_en', 'title_bn', 'title_slug_en', 'title_slug_bn', 'title_slug_en', 'title_slug_bn', 'thumbnail', 'created_at')->first();

        // favourite post
        $favourites = Post::with(['category:id,category_name_bn,category_name_en', 'user:id,name'])->where('status', 1)->inRandomOrder()->select('id', 'category_id', 'user_id', 'title_slug_en', 'title_slug_bn', 'title_en', 'title_bn', 'thumbnail', 'created_at')->limit(3)->get();

        // recent news
        $recent_news = Post::with(['category:id,category_name_bn,category_name_en', 'user:id,name'])->where('status', 1)->latest('id')->select('id', 'category_id', 'user_id', 'title_en', 'title_bn', 'title_slug_en', 'title_slug_bn', 'thumbnail', 'created_at')->limit(3)->get();
        ...
        // all country news
        $allcountries = Post::with(['category:id,category_name_bn,category_name_en', 'user:id,name'])->where('status', 1)->whereNotNull('division_id')->latest('id')->select('id', 'category_id', 'user_id', 'title_en', 'title_slug_en', 'title_slug_bn', 'title_bn', 'thumbnail', 'created_at')->limit(12)->get();
