...
class PostDetailsController extends Controller
{
    ...
    {
        // post details
        $postdetails = Post::with(['category:id,category_name_bn,category_name_en,category_name_slug_bn,category_name_slug_en', 'subcategory:id,category_id,subcategory_name_bn,subcategory_name_en', 'user:id,name', 'division:id,division_name_bn,division_name_en', 'district:id,division_id,district_name_bn,district_name_en'])->where('title_slug_en', $slug)->orWhere('title_slug_bn', $slug)->first();
        ...
        // releted_post
        $releted_post = Post::with(['category:id,category_name_bn,category_name_en,category_name_slug_bn,category_name_slug_en', 'user:id,name'])->where('status', 1)->whereNot('title_slug_en', $slug)->orWhereNot('title_slug_bn', $slug)->inRandomOrder()->select('id', 'category_id', 'user_id', 'title_en', 'title_bn', 'title_slug_en', 'title_slug_bn', 'thumbnail', 'created_at', 'post_month')->latest('post_month')->limit(8)->get();
